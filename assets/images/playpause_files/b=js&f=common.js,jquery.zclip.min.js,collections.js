$$=function(selector){if(typeof selector=='undefined')return null;if(selector==='')return null;return(selector.substring(0,1)==='#')?document.getElementById(selector.substring(1)):document.querySelectorAll(selector);};$(document).ready(function(){(function isLocalStorageSupported()
{var testKey='test',storage=window.localStorage;try
{storage.setItem(testKey,'1');storage.removeItem(testKey);return"localStorage"in window&&window["localStorage"];}
catch(error)
{console.error('Storage not supported, please disable private browsing or use a browser with this functionality');document.getElementById("no_storage_layer").style.display="block";document.body.style.overflow="hidden";return false;}})();if(!gr.cookie('cookie_policy')){$("#cookies-alert").show();setTimeout(function(){insertCookiePolicy();},10000);}
$('#cookies-alert .close-cookies').on('click',function(){insertCookiePolicy();});if(ls.logged_data){var data=JSON.parse(ls.logged_data);$$(".avatar")[0].src=data.avatar;$$(".username")[0].innerHTML=data.login;$$(".gr_not-connected").forEach(function(a){a.style.display='none';});$$(".gr_connected").forEach(function(a){a.style.display='inline-block';});}
$(document).ajaxStart(function(){loader(true);}).ajaxComplete(function(){loader();});if(typeof gr.sponsor!="undefined"&&typeof gr.sponsor.coupon!="undefined")gr.sponsor.coupon.launch();var pngSizes=[16,24,32,64,128,256,512];$(".close_panel").on("click",function(){$$("#backWrapper").style.display="none";document.body.style.overflow="auto";});$("#sizeSlider").slider({min:1,max:7,value:4,step:1,change:function(event,ui){elem=$$("#pngSizeSelected");prev=parseInt(elem.innerHTML,10);next=pngSizes[ui.value-1];counter(prev,next,elem);},stop:function(event,ui){elem=$$("#pngSizeSelected");prev=parseInt(elem.innerHTML,10);next=pngSizes[ui.value-1];counter(prev,next,elem);}});if(typeof downloadPack!=="undefined"){fixButton();$(document).scroll(function(){fixButton();});}
...
$("#fileFont").change(function(){cambiaNameFont();});$("#svgGenerator form").on("submit",function(e){e.preventDefault();$("#errorCollection").html("");if(compruebaFont()){collectionFromFont();}else{console.log("maaal");}});$("#downloadCollection").on("click",function(e){sURL=document.URL.split("/");cId=sURL[sURL.length-1];if(!gr.auth.logged){window.location.href=ACCOUNTS_BASE_URL;}else{$(".quickDownload").each(function(){$(this).attr("disabled","disabled");});var datos=$$("#dataColl").value,download_type="all",download_data=datos,download_size=4;execute_download();}});$(document).on("click",".external",function(e){e.preventDefault();window.open($(this).attr("href"));});$('body').keyup(function(e){if(e.which===27)displayModal(false);});$(document).on("click",".close-modal",function(){displayModal(false)});$(document).mousedown(function(e){var clicked=$(e.target);if(clicked.parents().is('.modal-overlay'))return;else displayModal(false);});$(".quickDownload").on("click",function(e){e.preventDefault();$(".quickDownload").each(function(){$(this).attr("disabled","disabled");});download_type=this.id;download_data=fi[current].iconos;download_data.download_type='collection';download_data=JSON.stringify(download_data);download_size=$("#sizeSlider").slider("option","value");displayModal(true);});$("#btn_attr_download").on("click",function(e){e.preventDefault();execute_download();});$("#give-break").change(function(){if($(this).is(':checked')){var date=new Date();localStorage['break_while']=date.setDate(date.getDate()+1);}else localStorage.removeItem('break_while');});});function getUserType(){if(!userId||typeof userId=='undefined')
return'Guest';else{return(gr.auth.me.premium)?'Premium':'Free';}}
function triggerPackDownload(){var pack_name=$("#pack_name").html(),pack_icons=parseInt($("#pack_icons").html(),10);_gaq.push(['_trackEvent','download','pack',pack_name,pack_icons]);_gaq.push(['_trackEvent','Downloads by user type',getUserType(),'Pack',pack_icons]);window.location.href=$("#downloadPackBtn").data("href");}
function insertCookiePolicy(){var date=new Date();date.setFullYear(date.getFullYear()+1);document.cookie="cookie_policy=true; expires="+date+"; path=/";$("#cookies-alert").slideUp();}
function eventSingleDownload(){var sizes=[16,24,32,64,128,256,512,'svg'],size=(download_type=='base64'||download_type=='png')?sizes[parseInt(download_size,10)-1]:'',singleDownload=(download_type!='base64')?download_type.toUpperCase()+size:(download_type+" "+size);_gaq.push(['_trackEvent','download','single',singleDownload,1]);_gaq.push(['_trackEvent','downloadSingleIcon',singleDownload]);_gaq.push(['_trackEvent','Downloads by user type',getUserType(),singleDownload,1]);}
function displayModal(action,hidePremium){if(action){$$(".join-premium")[0].style.display=($$("#detail_selection")&&hidePremium===true&&!Boolean(parseInt($$("#detail_selection").dataset.selection,10)))?'none':'block';}
if(action){if(new Date()<new Date(parseInt(ls.break_while,10))||gr.auth.me.premium){execute_download();return;}
getAttribution();$('#modal-overlay').fadeIn().addClass('modal-visible');}else{$('.modal-overlay').removeClass('modal-visible').fadeOut();$(".quickDownload").each(function(){$(this).removeAttr("disabled");});}}
function loader(display){$$("#loader").style.display=(display)?'block':'none';document.body.style.overflow=(!display)?'auto':'hidden';}
function getAttribution(){$.ajax({type:"POST",url:dom+"/request/getAttribution.php",global:false,data:"data="+download_data,success:function(result){$$("#attrTxtArea").value='<div>Icons made by '+result+' from <a href="http://www.flaticon.com" title="Flaticon">www.flaticon.com</a> \
...
function execute_download_old(){fbq('track','AddToCart');console.log('exe old.');displayModal(false);var dd=JSON.parse(download_data);var allowed=["svg","png","psd","eps"];if(Object.keys(dd).length==1&&allowed.indexOf(download_type)!=-1){track_search();trdownload(download_type);eventSingleDownload();window.location.href="/download/?dt="+download_type+"&ds="+download_size+"&dd="+download_data+"&uId="+userId;return;}
$.ajax({type:"POST",url:dom+"/request/download.php",data:"acc="+download_type+"&data="+download_data+"&scode="+download_size+"&uId="+userId,success:function(ret){console.log('ax ret',ret);if(download_type=="base64"){var b64css2=b64css.replace("%%B64%%",ret),mime=(download_size<8)?"image/png":"image/svg+xml",pngSizes=[16,24,32,64,128,256,512,200];b64css2=b64css2.replace("%%MIME%%",mime);b64img2=b64img.replace("%%B64%%",ret);$$("#b64css").innerHTML=b64css2.replace(/%%SIZE%%/g,pngSizes[download_size-1]);$$("#b64Image").innerHTML=b64img2.replace("%%MIME%%",mime);$("#dialog-b64image").dialog("option","title",lang.b64);$("#dialog-b64image").dialog("open");if(download_size==8)$$("#svgVectorImg").style.display="block";else $$("#svgVectorImg").style.display="none";}
var sizes=[16,24,32,64,128,256,512];var download_event=(download_type=='png')?"PNG"+sizes[download_size-1]:download_type.toUpperCase();var ico_num=Object.keys(JSON.parse(download_data)).length;_gaq.push(['_trackEvent','download','collection',download_event,ico_num]);_gaq.push(['_trackEvent','Downloads by user type',getUserType(),download_event,ico_num]);singleDownload=false;if(ret!==""&&download_type!='base64'){trdownload(download_type);var n=new Image();n.src=dom+"/request/updateStatsG.php?acc="+download_type+"&data="+download_data+"&uId="+userId;n.onload=function(){window.location.href="/download/?t="+ret+"&n="+fi[current].nombre;};n.onerror=function(){window.location.href="/download/?t="+ret+"&n="+fi[current].nombre;};}},error:function(err){console.error('Download error.',browser,version,detail);}});}
function execute_download(){fbq('track','AddToCart');displayModal(false);var dd=JSON.parse(download_data);var allowed=["svg","png","psd","eps"];var detail=jQuery('#detail .wrapper');var browser=flaticon_detectBrowser();var version=flaticon_detectVersion();var download_data_parsed=JSON.parse(download_data);var collection=(download_data_parsed.download_type=='collection')?true:false;if((download_type=='png'||download_type=='base64')&&detail.length==1&&Object.keys(dd).length==1){if(((browser=='IE')&&(download_type=='base64'))||((browser=='IE')&&(document.documentMode<11))||((download_type=='base64')&&(browser=='Safari')&&(version<8))||collection){execute_download_old();}else{track_search();trdownload(download_type);eventSingleDownload();var n=new Image();n.src=dom+"/request/updateStatsG.php?acc="+download_type+"&data="+download_data+"&uId="+userId;n.onload=function(){flaticon_svg2png();};n.onerror=function(){flaticon_svg2png();};return;}}
if(Object.keys(dd).length==1&&allowed.indexOf(download_type)!=-1){track_search();trdownload(download_type);eventSingleDownload();var n=new Image();n.src=dom+"/request/updateStatsG.php?acc="+download_type+"&data="+download_data+"&uId="+userId;n.onload=function(){window.location.href="/download/?dt="+download_type+"&ds="+download_size+"&dd="+download_data+"&uId="+userId;};n.onerror=function(){window.location.href="/download/?dt="+download_type+"&ds="+download_size+"&dd="+download_data+"&uId="+userId;};return;}
$.ajax({type:"POST",url:dom+"/request/download.php",data:"acc="+download_type+"&data="+download_data+"&scode="+download_size+"&uId="+userId,success:function(ret){console.log('AX',ret);if(download_type=="base64"&&false){var b64css2=b64css.replace("%%B64%%",ret),mime=(download_size<8)?"image/png":"image/svg+xml",pngSizes=[16,24,32,64,128,256,512,200];b64css2=b64css2.replace("%%MIME%%",mime);b64img2=b64img.replace("%%B64%%",ret);$$("#b64css").innerHTML=b64css2.replace(/%%SIZE%%/g,pngSizes[download_size-1]);$$("#b64Image").innerHTML=b64img2.replace("%%MIME%%",mime);$("#dialog-b64image").dialog("option","title",lang.b64);$("#dialog-b64image").dialog("open");if(download_size==8)$$("#svgVectorImg").style.display="block";else $$("#svgVectorImg").style.display="none";}
var sizes=[16,24,32,64,128,256,512];var download_event=(download_type=='png')?"PNG"+sizes[download_size-1]:download_type.toUpperCase();var ico_num=Object.keys(JSON.parse(download_data)).length;_gaq.push(['_trackEvent','download','collection',download_event,ico_num]);_gaq.push(['_trackEvent','Downloads by user type',getUserType(),download_event,ico_num]);if(ret!==""&&download_type!='base64'){trdownload(download_type);var n=new Image();n.src=dom+"/request/updateStatsG.php?acc="+download_type+"&data="+download_data+"&uId="+userId;n.onload=function(){window.location.href="/download/?t="+ret+"&n="+fi[current].nombre;};n.onerror=function(){window.location.href="/download/?t="+ret+"&n="+fi[current].nombre;};}
...
function sortBy(type,elem){var container=$('.centradora-div');elem=(elem===undefined)?".glyph-container":(elem.charAt(0)!=".")?"."+elem:elem;type=(type=='a')?'id':'data-'+type;container.each(function(){var elems=$(this).children(elem);elems.sort(function(a,b){var an=a.getAttribute(type),bn=b.getAttribute(type);an=(isNaN(an))?an:parseFloat(an);bn=(isNaN(bn))?bn:parseFloat(bn);if(an>bn)return-1;if(an<bn)return 1;return 0;});elems.detach().appendTo($(this));});}
...
function displayBugModal(){$('#modal-overlay-bug').fadeIn().addClass('modal-visible');}
...
defineElementGetter(Element.prototype,'classList',function(){return new DOMTokenList(this);});})();(function(){window.grconsole={};function formatArgs(args){var data=[].slice.call(arguments);data=(data.length>1)?data:data[0];return data;}
function traceLog(args){for(var i in args){if(args[i].hasOwnProperty('search')){if(args[i].search('SHA-1')!=-1||args[i].search('fb-root')!=-1)return false;}}
return true;}
if(console==undefined)return;if(console.log!=undefined){grconsole.log=[];console.olog=console.log;console.log=function(){if(!traceLog(arguments))return;window.grconsole.log.push(formatArgs(arguments));console.olog.apply(console,arguments);}}
if(console.warn!=undefined){grconsole.warn=[];console.owarn=console.warn;console.warn=function(){if(!traceLog(arguments))return;window.grconsole.warn.push(formatArgs(arguments));console.owarn.apply(console,arguments);}}
if(console.error!=undefined){grconsole.error=[];console.oerror=console.error;console.error=function(){if(!traceLog(arguments))return;window.grconsole.error.push(formatArgs(arguments));console.oerror.apply(console,arguments);}}
if(console.trace!=undefined){grconsole.trace=[];console.otrace=console.trace;console.trace=function(){if(!traceLog(arguments))return;window.grconsole.trace.push(formatArgs(arguments));console.otrace.apply(console,arguments);}}
if(console.info!=undefined){grconsole.info=[];console.oinfo=console.info;console.info=function(){if(!traceLog(arguments))return;window.grconsole.info.push(formatArgs(arguments));console.oinfo.apply(console,arguments);}}})();function traceBugReport(){var data={msg:''};data.navigator="";data.navigator+="User-agent: "+navigator.userAgent+"\n";data.navigator+="Platform: "+navigator.platform+"\n";data.navigator+="Browser Language: "+navigator.language+"\n";data.navigator+="Cookies Enabled: "+navigator.cookieEnabled+"\n";data.navigator+="Browser Online: "+navigator.onLine+"\n";var msgElem=document.getElementById('bugReportMessage');if(msgElem){data.msg=msgElem.value;msgElem.value='';if(data.msg==''){$('#modal-overlay-bug').fadeOut().removeClass('modal-visible');return;}};_gaq.push(['_trackEvent','Bug report']);if(typeof document.querySelector=='function'){var script=document.querySelector("script[src*='?ch=']").src.split("ch=")[1]}
data.console='';for(var k in grconsole){if(!grconsole[k].length)delete grconsole[k];}
if(typeof navigator.plugins!="undefined"){grconsole.plugins=[];[].forEach.call(navigator.plugins,function(e){grconsole.plugins.push(e.name)});}
data.console=(grconsole)?JSON.stringify(grconsole):'';data.language=language;data.download_data=window.download_data||'';data.download_format=window.download_type||'';data.url=window.location.href;data.script_version=script||'';$.ajax({global:false,method:'POST',url:dom+'/request/track_bug.php',dataType:'json',data:data,complete:function(){$('#modal-overlay-bug').fadeOut().removeClass('modal-visible');grconsole={log:[],info:[],warn:[],error:[],trace:[]};}});};(function(a){a.fn.zclip=function(c){if(typeof c=="object"&&!c.length){var b=a.extend({path:"ZeroClipboard.swf",copy:null,beforeCopy:null,afterCopy:null,clickAfter:true,setHandCursor:true,setCSSEffects:true},c);return this.each(function(){var e=a(this);if(e.is(":visible")&&(typeof b.copy=="string"||a.isFunction(b.copy))){ZeroClipboard.setMoviePath(b.path);var d=new ZeroClipboard.Client();if(a.isFunction(b.copy)){e.bind("zClip_copy",b.copy)}if(a.isFunction(b.beforeCopy)){e.bind("zClip_beforeCopy",b.beforeCopy)}if(a.isFunction(b.afterCopy)){e.bind("zClip_afterCopy",b.afterCopy)}d.setHandCursor(b.setHandCursor);d.setCSSEffects(b.setCSSEffects);d.addEventListener("mouseOver",function(f){e.trigger("mouseenter")});d.addEventListener("mouseOut",function(f){e.trigger("mouseleave")});d.addEventListener("mouseDown",function(f){e.trigger("mousedown");if(!a.isFunction(b.copy)){d.setText(b.copy)}else{d.setText(e.triggerHandler("zClip_copy"))}if(a.isFunction(b.beforeCopy)){e.trigger("zClip_beforeCopy")}});d.addEventListener("complete",function(f,g){if(a.isFunction(b.afterCopy)){e.trigger("zClip_afterCopy")}else{if(g.length>500){g=g.substr(0,500)+"...\n\n("+(g.length-500)+" characters not shown)"}e.removeClass("hover");alert("Copied text to clipboard:\n\n "+g)}if(b.clickAfter){e.trigger("click")}});d.glue(e[0],e.parent()[0]);a(window).bind("load resize",function(){d.reposition()})}})}else{if(typeof c=="string"){return this.each(function(){var f=a(this);c=c.toLowerCase();var e=f.data("zclipId");var d=a("#"+e+".zclip");if(c=="remove"){d.remove();f.removeClass("active hover")}else{if(c=="hide"){d.hide();f.removeClass("active hover")}else{if(c=="show"){d.show()}}}})}}}})(jQuery);var ZeroClipboard={version:"1.0.7",clients:{},moviePath:"ZeroClipboard.swf",nextId:1,$:function(a){if(typeof(a)=="string"){a=document.getElementById(a)}if(!a.addClass){a.hide=function(){this.style.display="none"};a.show=function(){this.style.display=""};a.addClass=function(b){this.removeClass(b);this.className+=" "+b};a.removeClass=function(d){var e=this.className.split(/\s+/);var b=-1;for(var c=0;c<e.length;c++){if(e[c]==d){b=c;c=e.length}}if(b>-1){e.splice(b,1);this.className=e.join(" ")}return this};a.hasClass=function(b){return!!this.className.match(new RegExp("\\s*"+b+"\\s*"))}}return a},setMoviePath:function(a){this.moviePath=a},dispatch:function(d,b,c){var a=this.clients[d];if(a){a.receiveEvent(b,c)}},register:function(b,a){this.clients[b]=a},getDOMObjectPosition:function(c,a){var b={left:0,top:0,width:c.width?c.width:c.offsetWidth,height:c.height?c.height:c.offsetHeight};if(c&&(c!=a)){b.left+=c.offsetLeft;b.top+=c.offsetTop}return b},Client:function(a){this.handlers={};this.id=ZeroClipboard.nextId++;this.movieId="ZeroClipboardMovie_"+this.id;ZeroClipboard.register(this.id,this);if(a){this.glue(a)}}};ZeroClipboard.Client.prototype={id:0,ready:false,movie:null,clipText:"",handCursorEnabled:true,cssEffects:true,handlers:null,glue:function(d,b,e){this.domElement=ZeroClipboard.$(d);var f=99;if(this.domElement.style.zIndex){f=parseInt(this.domElement.style.zIndex,10)+1}if(typeof(b)=="string"){b=ZeroClipboard.$(b)}else{if(typeof(b)=="undefined"){b=document.getElementsByTagName("body")[0]}}var c=ZeroClipboard.getDOMObjectPosition(this.domElement,b);this.div=document.createElement("div");this.div.className="zclip";this.div.id="zclip-"+this.movieId;$(this.domElement).data("zclipId","zclip-"+this.movieId);var a=this.div.style;a.position="absolute";a.left=""+c.left+"px";a.top=""+c.top+"px";a.width=""+c.width+"px";a.height=""+c.height+"px";a.zIndex=f;if(typeof(e)=="object"){for(addedStyle in e){a[addedStyle]=e[addedStyle]}}b.appendChild(this.div);this.div.innerHTML=this.getHTML(c.width,c.height)},getHTML:function(d,a){var c="";var b="id="+this.id+"&width="+d+"&height="+a;if(navigator.userAgent.match(/MSIE/)){var e=location.href.match(/^https/i)?"https://":"http://";c+='<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="'+e+'download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0" width="'+d+'" height="'+a+'" id="'+this.movieId+'" align="middle"><param name="allowScriptAccess" value="always" /><param name="allowFullScreen" value="false" /><param name="movie" value="'+ZeroClipboard.moviePath+'" /><param name="loop" value="false" /><param name="menu" value="false" /><param name="quality" value="best" /><param name="bgcolor" value="#ffffff" /><param name="flashvars" value="'+b+'"/><param name="wmode" value="transparent"/></object>'}else{c+='<embed id="'+this.movieId+'" src="'+ZeroClipboard.moviePath+'" loop="false" menu="false" quality="best" bgcolor="#ffffff" width="'+d+'" height="'+a+'" name="'+this.movieId+'" align="middle" allowScriptAccess="always" allowFullScreen="false" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" flashvars="'+b+'" wmode="transparent" />'}return c},hide:function(){if(this.div){this.div.style.left="-2000px"}},show:function(){this.reposition()},destroy:function(){if(this.domElement&&this.div){this.hide();this.div.innerHTML="";var a=document.getElementsByTagName("body")[0];try{a.removeChild(this.div)}catch(b){}this.domElement=null;this.div=null}},reposition:function(c){if(c){this.domElement=ZeroClipboard.$(c);if(!this.domElement){this.hide()}}if(this.domElement&&this.div){var b=ZeroClipboard.getDOMObjectPosition(this.domElement);var a=this.div.style;a.left=""+b.left+"px";a.top=""+b.top+"px"}},setText:function(a){this.clipText=a;if(this.ready){this.movie.setText(a)}},addEventListener:function(a,b){a=a.toString().toLowerCase().replace(/^on/,"");if(!this.handlers[a]){this.handlers[a]=[]}this.handlers[a].push(b)},setHandCursor:function(a){this.handCursorEnabled=a;if(this.ready){this.movie.setHandCursor(a)}},setCSSEffects:function(a){this.cssEffects=!!a},receiveEvent:function(d,f){d=d.toString().toLowerCase().replace(/^on/,"");switch(d){case"load":this.movie=document.getElementById(this.movieId);if(!this.movie){var c=this;setTimeout(function(){c.receiveEvent("load",null)},1);return}if(!this.ready&&navigator.userAgent.match(/Firefox/)&&navigator.userAgent.match(/Windows/)){var c=this;setTimeout(function(){c.receiveEvent("load",null)},100);this.ready=true;return}this.ready=true;try{this.movie.setText(this.clipText)}catch(h){}try{this.movie.setHandCursor(this.handCursorEnabled)}catch(h){}break;case"mouseover":if(this.domElement&&this.cssEffects){this.domElement.addClass("hover");if(this.recoverActive){this.domElement.addClass("active")}}break;case"mouseout":if(this.domElement&&this.cssEffects){this.recoverActive=false;if(this.domElement.hasClass("active")){this.domElement.removeClass("active");this.recoverActive=true}this.domElement.removeClass("hover")}break;case"mousedown":if(this.domElement&&this.cssEffects){this.domElement.addClass("active")}break;case"mouseup":if(this.domElement&&this.cssEffects){this.domElement.removeClass("active");this.recoverActive=false}break}if(this.handlers[d]){for(var b=0,a=this.handlers[d].length;b<a;b++){var g=this.handlers[d][b];if(typeof(g)=="function"){g(this,f)}else{if((typeof(g)=="object")&&(g.length==2)){g[0][g[1]](this,f)}else{if(typeof(g)=="string"){window[g](this,f)}}}}}}};;var conn=new Communicator(),current,change=false,tempVar,mergedLibrary=true;var ls=localStorage,ss=sessionStorage,byId=function(id){return document.getElementById(id);},byClass=function(className){return document.getElementsByClassName(className);},byTag=function(tag){return document.getElementsByTagName(tag);};function Communicator(){}
...
if(!ignore){current=tempVar;localStorage.current=tempVar;this.refresh(true);}};Library.prototype.del=function(id){if(typeof id=="undefined")return;delete this[id];if(sessionStorage.loading==="false"||typeof sessionStorage.loading==="undefined")
...
$(function(){setTimeout(initLib,4000);});var initLib=function(){fi=new Library();if(typeof gr.auth=="undefined"||!gr.auth.logged)
...
{if(data.status)
{ls.logged_data=JSON.stringify({login:data.login,avatar:data.avatar});$$(".avatar")[0].src=data.avatar;$$(".username")[0].innerHTML=data.login;$$(".gr_not-connected").forEach(function(a){a.style.display='none';});$$(".gr_connected").forEach(function(a){a.style.display='inline-block';});var btn=$$("#downloadPackBtn");if(btn)btn.setAttribute("data-href",btn.getAttribute("data-href")+"&uId="+data.id);}
else
{if(ls.logged_data)
